...
import { verifiedUpstreamMetadata } from "./src/core/upstream-capabilities.mjs";
...
function listModels(config, capabilities) {
  return Object.entries(config.routes).map(([id, route]) => {
    ...
    const models = entries.map((entry) => {
      ...
      const verified = verifiedUpstreamMetadata(model);
      const declaredCapabilities = typeof entry === "object" && entry.capabilities
        ? entry.capabilities
        : {};
      const declaredReasoningEfforts = typeof entry === "object"
        ? entry.reasoning_efforts
        : undefined;
      const inputModalities = verified?.input_modalities
        ?? info?.raw?.input_modalities
        ?? (typeof entry === "object" ? entry.input_modalities : undefined);
      const outputModalities = verified?.output_modalities
        ?? info?.raw?.output_modalities
        ?? (typeof entry === "object" ? entry.output_modalities : undefined);
      ...
      if (!verified
        && !useDiscoveredCapabilities
        && declaredCapabilities.reasoning === undefined
        && Array.isArray(declaredReasoningEfforts)
        && declaredReasoningEfforts.length > 0) {
        mergedCapabilities.reasoning = true;
      }
      const discoveredRuntime = runtimeForModel(capabilities, provider, model);
      const verifiedRuntime = verified
        ? runtimeMetadata(verified, mergedCapabilities)
        : undefined;
      const declaredRuntime = runtimeMetadata({
        context_lengths: typeof entry === "object" ? entry.context_lengths : undefined,
        max_input_tokens: typeof entry === "object" ? entry.max_input_tokens : undefined,
        max_context_tokens: typeof entry === "object" ? entry.max_context_tokens : undefined,
        reasoning_efforts: typeof entry === "object" ? entry.reasoning_efforts : undefined
      }, mergedCapabilities);
      const runtime = mergeRuntimeMetadata(
        verifiedRuntime,
        discoveredRuntime,
        declaredRuntime,
        mergedCapabilities
      ...
      return {
        ...
        ...(Array.isArray(inputModalities) && inputModalities.length
          ? { input_modalities: [...inputModalities] }
          : {}),
        ...(Array.isArray(outputModalities) && outputModalities.length
    ...
    return {
      ...
      ...(primary?.input_modalities
        ? { input_modalities: primary.input_modalities }
        : {}),
      ...(primary?.output_modalities
        ? { output_modalities: primary.output_modalities }
        : {}),
      ...runtimeFields({
        contextLengths: primary?.context_lengths,
        maxInputTokens: primary?.max_input_tokens,
        reasoningEfforts: primary?.reasoning_efforts,
        reasoningRequired: primary?.reasoning_required === true,
        reasoningDisabled: primary?.reasoning_disabled === false
          ? false
          : true
      }),
      fallback_models: models.slice(1).map(({
        provider,
        model,
        protocol: _fallbackProtocol,
...
function mergeRuntimeMetadata(verified, discovered, declared, capabilities) {
  const values = [verified, discovered, declared].filter(Boolean);
  const first = (field, fallback) => values.find((value) => (
    value[field] !== undefined
      && (!Array.isArray(value[field]) || value[field].length > 0)
  ))?.[field] ?? fallback;
  return {
    contextLengths: first("contextLengths", []),
    maxInputTokens: first("maxInputTokens"),
    reasoningEfforts: first("reasoningEfforts", []),
    reasoningRequired: first("reasoningRequired", false) === true,
    reasoningDisabled: first("reasoningDisabled", true) !== false,
...
function runtimeFields(runtime) {
  if (!runtime) return {};
  return {
    ...(runtime.contextLengths?.length
      ? { context_lengths: runtime.contextLengths }
      : {}),
    ...(runtime.maxInputTokens
      ? { max_input_tokens: runtime.maxInputTokens }
      : {}),
    ...(runtime.reasoningEfforts?.length
      ? { reasoning_efforts: runtime.reasoningEfforts }
      : {}),
    ...(runtime.reasoningRequired
      ? { reasoning_required: true }
      : {}),
    ...(runtime.reasoningDisabled === false
